...
function fetchLogs() {
  ...
  // dwell fetch RIÊNG (kèm cột dwell_s): nếu chưa chạy migrate_05 → query này
  // lỗi và trả [] nhưng dashboard chính vẫn nguyên vẹn
  var baseD = '/rest/v1/events?select=' + encodeURIComponent(sel + ',dwell_s');
  ...
  return Promise.all([
    ...
    fetchParallel(baseD + '&pos=eq.dwell&order=ts.desc',         2)
  ])
...
function process(logs){
  ...
  for(var i=0;i<logs.length;i++){
    ...
    // dwell = số giây pixel bottom giữ kết nối ≈ thời gian đọc; proxy tải hộ → bỏ
    if(l.pos==='dwell'&&l.dwell_s!=null&&deviceOf(l.ua||'')!=='proxy'){s.dwells.push(+l.dwell_s||0);}
  ...
  Object.keys(sess).forEach(function(k){
    var s=sess[k];
    // Thời gian đọc của session = lần xem DÀI NHẤT (mở nhiều lần → lấy max)
...
function campaignSection(d){
  ...
  return '<section id="s-camp"><div class="eyebrow">Hiệu quả chiến dịch — bấm dòng xem chi tiết '+qclearBtn()+'</div>'
    ...
    +th('camp','medRead','Đọc TB','num','Median thời gian đọc (dwell) — đo từ pixel streaming, đã loại proxy, cap ~25s/lượt')
...
/* ── Read time panel ── thời gian đọc từ dwell (pixel bottom streaming) ── */
function readTimePanel(d){
  var s=d.sum;
  if(!s.readN)return '';
  var total=s.readN||1;
  function fr(label,n,color,tip){
...
function operational(d,cur,prev,ser){
  ...
  return masthead('op')+sub+filterStatusBar()+
    ...
    (d.sum.readN>0?'<div style="margin-top:16px">'+readTimePanel(d)+'</div>':'')+
