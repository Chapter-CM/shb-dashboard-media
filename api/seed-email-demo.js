// Endpoint TẠM để seed dữ liệu demo cho Email dashboard, chạy trong môi trường Vercel
// (đã có sẵn EMAIL_SUPABASE_URL / EMAIL_SUPABASE_SERVICE_KEY trong env, không cần dán key).
// Mở trên trình duyệt: /api/seed-email-demo?token=demo-2026
// Xoá data demo: /api/seed-email-demo?token=demo-2026&wipe=1
// ⚠️ Xoá file này khỏi repo sau khi demo xong — đây là công cụ dùng 1 lần.
'use strict';
...
const TOKEN = process.env.SEED_TOKEN || 'demo-2026';
...
function buildPeople(n) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    used.add(local);
    people.push({ rcpt: local + '@shb.com.vn', dept: pick(DEPTS), role: pick(ROLES) });
  }
  return people;
}
...
function buildRowsForCampaign(camp, people) {
  ...
  targets.forEach((p) => {
    ...
    if (!pct(camp.reach)) return;
    const openTs = sendTs + (5 + rnd(240)) * 60e3;
...
module.exports = async (req, res) => {
  ...
  if (q.token !== TOKEN) {
    res.statusCode = 403;
    return res.end(JSON.stringify({ error: 'forbidden — thêm ?token=... đúng giá trị' }));
  }
  if (!SUPABASE_URL || !SERVICE_KEY) {
    res.statusCode = 500;
    return res.end(JSON.stringify({ error: 'Thiếu EMAIL_SUPABASE_URL / EMAIL_SUPABASE_SERVICE_KEY trên Vercel' }));
  }
  try {
    if (q.wipe) {
      await req_wipe();
    ...
    for (let i = 0; i < allRows.length; i += BATCH) {
      await supaReq('POST', '/rest/v1/events', allRows.slice(i, i + BATCH));
    }
    res.end(JSON.stringify({
      ...
      wiped: !!q.wipe,
      next: 'Mở /api/email hoặc /api/leader để xem demo (hard refresh nếu đã mở sẵn).',
    }, null, 2));
  } catch (e) {
    res.statusCode = 500;
  ...
  function req_wipe() {
    return supaReq('DELETE', '/rest/v1/events?event_id=like.seed*', null);
  }
};
