...
async function insert(table, rows, opts = {}) {
  const built = buildInsert(table, rows, opts);
  if (!built) return 0;
  const [res] = await getPool().query(built.sql, [built.values]);
...
module.exports = { isEnabled, get, parsePath, getPool, end, insert, buildInsert };
